...
describe('CommunityService', () => {
  ...
  describe('createClan', () => {
    ...
    it('should create clan and member', async () => {
      mockPrisma.clan.create.mockResolvedValue({
        id: 'clan1',
        name: 'Test Clan',
        slug: 'test-clan-abc',
        visibility: 'public',
      ...
      expect(mockEvents.emit).toHaveBeenCalledWith('clan.created', {
        clanId: 'clan1',
        leaderId: 'user1',
      });
    });
  ...
  describe('requestJoin', () => {
    it('should throw if clan not found', async () => {
      ...
      await expect(service.requestJoin('user1', 'clan1')).rejects.toThrow(
        'Clan not found',
      );
    });

    it('should throw if already active member', async () => {
      mockPrisma.clan.findUnique.mockResolvedValue({
        id: 'clan1',
        visibility: 'public',
      });
    ...
    it('should update existing rejected request to pending', async () => {
      ...
      mockPrisma.clanMember.findUnique.mockResolvedValue(null);
      mockPrisma.clanJoinRequest.findFirst.mockResolvedValue({
        id: 'req1',
        status: 'rejected',
      });
  ...
  it('should throw for invalid clan name', () => {
    expect(() => service.validateClanName('')).toThrow(
      'Clan name must be at least 3 characters',
    );
    expect(() => service.validateClanName('ab')).toThrow(
      'Clan name must be at least 3 characters',
    );
  });
